            ...
            Games.map((game, index) => {
              return (
                <Link
                  ...
                  className="flex h-1/3 space-x-6 md:space-x-12 p-3 shadow-md md:shadow-lg border-[2px] hover:bg-blue-200 active:bg-blue-300 rounded-2xl w-full cursor-pointer no-underline"
                >
                  <img
                    src={game.imgGame}
                    alt=""
                    className="w-1/3 h-auto rounded-lg md:rounded-xl"
                  />
                  <div className="flex w-full items-center">
                    <h1 className="text-gray-600 text-2xl md:text-3xl">
                      {game.name}
                    </h1>
                  </div>
                </Link>
        ...
        <div className="hidden lg:flex flex-col items-center px-6 lg:px-14 py-4 lg:space-y-10">
          ...
          <div className="w-full grid grid-cols-4 2xl:grid-cols-5 gap-8 px-4">
              ...
              Games.map((game, index) => {
                return (
                  <div key={index} className="flex flex-col items-center">
                    <h1 className="text-2xl text-gray-600">{game.name}</h1>
                    <Link
                      to="/games/category"
                      state={{ gameName: game.name }}
                      className="flex w-full h-auto border-[2px] p-3 rounded-3xl shadow-xl hover:bg-blue-200 active:bg-blue-300 cursor-pointer"
                    >
                      <img
                        src={game.imgGame}
                        alt=""
            ...
            <div className="flex flex-col items-center">
              <h1 className="text-2xl text-gray-600">{simulationGame.name}</h1>
              <Link
                to="/games/category/simulationGame"
                className="flex border-[2px] p-3 rounded-3xl shadow-xl hover:bg-blue-200 active:bg-blue-300 cursor-pointer"
              >
                <img
                  src={simulationGame.imgGame}
                  alt=""
